...
export default function JoinQueue({
  ...
  const cancelQueue = useMutation(api.waitingList.cancelQueue);
  const skipUser = useMutation(api.waitingList.skipUser);
    ...
    <div>
      {queuePosition && 
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between w-full py-3 px-4 bg-gray-100 rounded-lg">
            <div className="flex items-center gap-2">
              <Clock className="w-5 h-5" />
              <span>Position in queue: {queuePosition.position}</span>
            </div>
            <button
              onClick={async () => {
                try {
                  await cancelQueue({ eventId, userId });
                  toast({
                    title: "Success",
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  toast({
                    title: "Success",
                    description: "User skipped in queue",
                  });
                } catch (err) {
                  ...
                  toast({
                    variant: "destructive",
                    title: "Error",
                    description: "Failed to skip user",
                  });
                }
              }}
